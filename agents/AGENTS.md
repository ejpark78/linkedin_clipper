...
* **일관된 릴리즈 및 이슈 자동 종결**: 작업 완료 후 `npm run git commit`으로 일괄 처리합니다.
* **항상 `npm run git commit`으로 마감**합니다.
...
* **리스크 사전 고지 의무**: 계획 설명 시 데이터 손실, 범위 외 영향, 타 에이전트 작업과의 충돌 등 예상되는 위험을 반드시 사용자에게 알린 후 승인을 받습니다.
* **투명한 이슈 처리**: 오류 즉시 보고, 무음 복구 금지.
* **명령어 실행 에러 공개**: 모든 비정상 종료를 사용자에게 공개합니다.
* **AI 처리 및 응답 한국어**: 모든 AI 로그, 상태 메시지, 채팅 응답은 한국어.
* **대화 로깅**: 세션 중 모든 요청/응답을 `agents/data/chat/YYYY-mm-dd.md`에 날짜별로 기록합니다. `agents/data/`는 `.gitignore`에 포함되어 로컬 전용이며 커밋되지 않습니다.
